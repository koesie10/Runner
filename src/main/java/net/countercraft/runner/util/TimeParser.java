package net.countercraft.runner.util;
 ...
 * 
 * Originally made by Olof Larsson for Essentials
 * 
 * http://www.minecraftwiki.net/wiki/Day/night_cycle
 * 
 * @author Olof Larsson
 * @link 
 *       https://github.com/essentials/Essentials/blob/master/Essentials/src/net/
 *       ess3/utils/DescParseTickFormat.java
 */
public class TimeParser {
	...
	private static final SimpleDateFormat SDFTwentyFour = new SimpleDateFormat(
			"HH:mm", Locale.ENGLISH);
	private static final SimpleDateFormat SDFTwelve = new SimpleDateFormat(
			"h:mmaa", Locale.ENGLISH);
	...
	public static long parse(String desc) throws NumberFormatException {
		...
		/*
		 * // Detect 24-hour format try { return parse24(desc); } catch
		 * (Exception e) { }
		 * 
		 * // Detect 12-hour format try { return parse12(desc); } catch
		 * (Exception e) { }
		 */
	...
	public static String format(final long ticks) {
		return format24(ticks) + ", " + format12(ticks) + ", "
				+ formatTicks(ticks);
	}
	...
	public static String formatDateFormat(final long ticks,
