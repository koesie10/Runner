...
public class XMPPManager {
					...
					if (!createdLocally && !chatExactlyExists(chat)) {
						if (Controller.getSettings().OPEN_CHANNEL) {
							try {
								...
								Logger.getLogger(XMPPManager.class.getName())
							...
							if (Controller.getSettings().isUser(
									correctAdress(chat.getParticipant()))) {
								try {
									chat.sendMessage("You have now been added to this server's chat room.");
								} catch (XMPPException ex) {
									Logger.getLogger(
											XMPPManager.class.getName()).log(
	...
	private void createAndAddToChat(Chat chat) {
		connectionList.add(chatManager.createChat(
				correctAdress(chat.getParticipant()), new InboundHandler()));
	}
